...
class _ActivityManageDetailsScreenState extends State<ActivityManageDetailsScreen> {
  ...
  final TextEditingController metaSummaryController = TextEditingController(); // used as Excerpt

  // HTML produced by ContentDetails (Quill Delta -> HTML).
  ...
  // Single featured/gallery image (sent as the one gallery_images entry)
  File? selectedImage;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<ActivityViewModel>().getStatusChoices();
  ...
  Future<void> _handleSaveActivity() async {
    ...
    final created = await viewModel.createActivity(data);
    ...
    if (created != null) {
      SnackBarMessage.showSnackBar(context, "Activity post created successfully");
      Navigator.pop(context, true);
    } else {
      SnackBarMessage.showSnackBar(context, viewModel.actionError ?? "Failed to create activity post");
    }
  }
              ...
              delegate: SliverChildListDelegate([
                Column(
                  children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (provider.categoryLoading)
                                ...
                                TextBodyStyleWidget(title: "No categories yet", size: AppSizes.cardTitle)
                              else
                                CustomDropdown(
                                  items: provider.categoryList.map((e) => e.name ?? "").toList(),
                                  initialValue: selectedCategoryId != null
                                      ? provider.categoryList.firstWhere((e) => e.id == selectedCategoryId).name
                                      : provider.categoryList.first.name,
                                  width: 100.w,
                                  onChanged: (value) {
                                    final match = provider.categoryList.firstWhere((e) => e.name == value);
                                    setState(() {
                    ...
                    // Activity Setting (Author + Status)
                    CustomCard(
                      child: Column(
                        children: [
                    ...
                    // SEO Setting (also carries Excerpt)
                    CustomCard(
                      child: InstituteOverviewScreen(
                        ...
                        expandableChild: InfrastructureDropDown(
                          fields: [
                            FormFieldData(title: "Meta Title", hint: "SEO Title", controller: metaTitleController),
                            FormFieldData(
                          ...
                          // Just collapse the panel — values stay in the controllers
                          // and are read directly when the main Save button is tapped.
                          onSave: () {
                            setState(() {
                      ...
                      children: [
                        ...
                        Flexible(
                          child: CustomButton(
                            ...
                            onTap: isSaving ? null : _handleSaveActivity,
                          ),
                        ),
                      ],
                    ),
